...
// Most recent event
$recent_event = $conn_events->query("SELECT event_name, created_at FROM upcoming_events WHERE status = 'active' ORDER BY id DESC LIMIT 1");
$event = $recent_event->fetch_assoc();

// Most recent gallery upload
$recent_gallery = $conn_gallery->query("SELECT title, uploaded_at FROM gallery ORDER BY id DESC LIMIT 1");
$gallery = $recent_gallery->fetch_assoc();

// Most recent news post
$recent_news = $conn_news->query("SELECT headline, posted_at FROM news_announcement WHERE status = 'active' ORDER BY news_id DESC LIMIT 1");
$news = $recent_news->fetch_assoc();

// Most recent inquiry
$recent_inquiry = $conn_contact->query("SELECT full_name, submitted_at FROM contact_inquiries WHERE archived = 0 ORDER BY registration_date DESC LIMIT 1");
