...
$recent_news = $conn_news->query("SELECT headline, news_id FROM news_announcements WHERE status = 'active' ORDER BY news_id DESC LIMIT 1");
$news = $recent_news->fetch_assoc();

// Most recent inquiry
$recent_inquiry = $conn_contact->query("SELECT full_name, registration_date FROM contact_inquiries WHERE archived = 0 ORDER BY registration_date DESC LIMIT 1");
