...
// Most recent event
$recent_event = $conn_events->query("SELECT event_name, created_at FROM upcoming_events WHERE status = 'active' ORDER BY created_at DESC LIMIT 1");
$event = $recent_event->fetch_assoc();

// Most recent gallery upload
$recent_gallery = $conn_gallery->query("SELECT title, uploaded_at FROM gallery ORDER BY uploaded_at DESC LIMIT 1");
$gallery = $recent_gallery->fetch_assoc();

// Most recent news post
$recent_news = $conn_news->query("SELECT news_title, publish_date FROM news_announcements WHERE status = 'active' ORDER BY publish_date DESC LIMIT 1");
