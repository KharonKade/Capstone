...
// Most recent event
$recent_event = $conn_events->query("SELECT event_name, created_at FROM upcoming_events WHERE status = 'active' ORDER BY created_at DESC LIMIT 5");
$event = $recent_event->fetch_assoc();

// Most recent gallery upload
$recent_gallery = $conn_gallery->query("SELECT title, uploaded_at FROM gallery ORDER BY uploaded_at DESC LIMIT 5");
$gallery = $recent_gallery->fetch_assoc();

// Most recent news post
$recent_news = $conn_news->query("SELECT news_title, publish_date FROM news_announcements WHERE status = 'active' ORDER BY publish_date DESC LIMIT 5");
$news = $recent_news->fetch_assoc();

// Most recent inquiry
$recent_inquiry = $conn_contact->query("SELECT full_name, submitted_at FROM contact_inquiries WHERE archived = 0 ORDER BY submitted_at DESC LIMIT 5");
...
<body>
    <div class="admin-container">
        ...
        <ul>
            <?php if (isset($event)) : ?>
                <li>✅ Event "<?php echo htmlspecialchars($event['event_name']); ?>" was added on <?php echo date("M d, Y", strtotime($event['created_at'])); ?></li>
            <?php endif; ?>

            <?php if (isset($gallery)) : ?>
                <li>📸 New gallery item "<?php echo htmlspecialchars($gallery['title']); ?>" uploaded on <?php echo date("M d, Y", strtotime($gallery['uploaded_at'])); ?></li>
            <?php endif; ?>

            <?php if (isset($news)) : ?>
                <li>📰 News article "<?php echo htmlspecialchars($news['news_title']); ?>" posted on <?php echo date("M d, Y", strtotime($news['publish_date'])); ?></li>
            <?php endif; ?>

            <?php if (isset($inquiry)) : ?>
                <li>❓ Inquiry from "<?php echo htmlspecialchars($inquiry['full_name']); ?>" received on <?php echo date("M d, Y", strtotime($inquiry['submitted_at'])); ?></li>
            <?php endif; ?>
        </ul>
    </div>
