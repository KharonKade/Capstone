...
<body>
    <div class="admin-container">
        ...
        <div class="recent-activity">
            <h3>Recent Activities (Week of <?= $monday->format('F j') ?> – <?= $sunday->format('F j') ?>)</h3>

            <!-- Events -->
            <div class="activity-section">
                <button class="toggle-btn">🗓️ Events ▼</button>
                <ul class="activity-list" style="display: none;">
                    <?php while($event = $recent_event->fetch_assoc()): ?>
                        <li><strong><?= $event['event_name'] ?></strong> — <?= date("F j, Y", strtotime($event['created_at'])) ?></li>
                    <?php endwhile; ?>
                </ul>
            </div>
            ...
            <!-- Gallery -->
            <div class="activity-section">
                <button class="toggle-btn">🖼️ Gallery ▼</button>
                <ul class="activity-list" style="display: none;">
                    <?php while($gallery = $recent_gallery->fetch_assoc()): ?>
                        <li><strong><?= $gallery['title'] ?></strong> — <?= date("F j, Y", strtotime($gallery['uploaded_at'])) ?></li>
                    <?php endwhile; ?>
                </ul>
            </div>
            ...
            <!-- News -->
            <div class="activity-section">
                <button class="toggle-btn">📰 News ▼</button>
                <ul class="activity-list" style="display: none;">
                    <?php while($news = $recent_news->fetch_assoc()): ?>
                        <li><strong><?= $news['news_title'] ?></strong> — <?= date("F j, Y", strtotime($news['publish_date'])) ?></li>
                    <?php endwhile; ?>
                </ul>
            </div>
            ...
            <!-- Inquiries -->
            <div class="activity-section">
                <button class="toggle-btn">📩 Inquiries ▼</button>
                <ul class="activity-list" style="display: none;">
                    <?php while($inq = $recent_inquiry->fetch_assoc()): ?>
                        <li><strong><?= $inq['full_name'] ?></strong> — <?= date("F j, Y", strtotime($inq['submitted_at'])) ?></li>
                    <?php endwhile; ?>
                </ul>
            </div>
        </div>
