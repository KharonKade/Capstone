...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($_FILES['athlete_image']['name'])) {
        ...
        if (!empty($_FILES['athlete_gallery']['name'][0])) {
            ...
            foreach ($_FILES['athlete_gallery']['name'] as $index => $gallery_img) {
                $gallery_filename = time() . "_" . basename($gallery_img);
                $gallery_target = "uploads/athletes/gallery/" . $gallery_filename;
                
                if (move_uploaded_file($_FILES['athlete_gallery']['tmp_name'][$index], $gallery_target)) {
                    $gallery_desc = htmlspecialchars($_POST['gallery_description'][$index]);
                    $stmt->bind_param("iss", $athlete_id, $gallery_filename, $gallery_desc);
                    $stmt->execute();
                }
            }
            $stmt->close();
        ...
        echo "Please upload an athlete image.";
    }
