...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (!empty($_FILES['athlete_gallery']['name'][0])) {
            foreach ($_FILES['athlete_gallery']['name'] as $key => $gallery_img) {
                ...
                if ($gallery_img && $temp_name) {
                    $target_path = "images/uploads/$gallery_img";
                    ...
                    $stmt->bind_param("iss", $athlete_id, $gallery_img, $_POST['gallery_description'][$key]);
                    $stmt->execute();
                }
            }
        }
    ...
    if (!empty($_FILES['leader_image']['name'])) {
        ...
        move_uploaded_file($_FILES['leader_image']['tmp_name'], "images/uploads/$leader_img");
    ...
    if (!empty($_FILES['partner_logo']['name'])) {
        ...
        move_uploaded_file($_FILES['partner_logo']['tmp_name'], "images/uploads/$partner_logo");
