        ...
        <main class="content">
            ...
            <section>
                ...
                while ($row = $result->fetch_assoc()) {
                    ...
                    // Editable fields
                    echo "<input type='text' id='title-{$row['id']}' value='{$row['title']}'>";
                    echo "<textarea id='desc-{$row['id']}'>{$row['description']}</textarea>";

                    // File Upload (Only when needed)
                    echo "<input type='file' id='video-{$row['id']}'>";
            ...
            function updateHighlight(id) {
                ...
                let description = document.getElementById(`desc-${id}`).value;
                let formData = new FormData();
                formData.append("id", id);
                formData.append("title", title);
                formData.append("description", description);

                let videoFile = document.getElementById(`video-${id}`).files[0];
                if (videoFile) {
