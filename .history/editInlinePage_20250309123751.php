        ...
        <main class="content">
            ...
            <section>
                ...
                while ($row = $result->fetch_assoc()) {
                    echo "<div id='highlight_{$row['id']}'>";
                    echo '<video src="' . $row["video"] . '" autoplay muted loop></video>';

                    // Editable fields (Inline Editing)
                    echo "<p contenteditable='true' id='title_{$row['id']}'>{$row['title']}</p>";
                    echo "<p contenteditable='true' id='desc_{$row['id']}'>{$row['description']}</p>";

                    // File input for video update
                    echo "<input type='file' id='video_{$row['id']}' style='display:none;'>";

                    // Buttons
                    echo "<button onclick=\"triggerFileUpload('video_{$row['id']}')\">Change Video</button>";
                    echo "<button onclick=\"updateHighlight({$row['id']})\">Update</button>";
                ...
                <script>
                    ...
                    function updateHighlight(id) {
                        let title = document.getElementById('title_' + id).innerText;
                        let desc = document.getElementById('desc_' + id).innerText;
                        let videoFile = document.getElementById('video_' + id).files[0];
