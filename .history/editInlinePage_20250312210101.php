        ...
        <main class="content">
            ...
            <section>
                ...
                while ($row = $result->fetch_assoc()) {
                    ...
                    echo "</div>";
                    echo "<button class='add-achievement' data-athlete='{$row['id']}'>Add More Achievements</button>";
                    echo "<div class='new-achievement-form' id='newAchievementForm{$row['id']}' style='display:none;'>";
                    echo "<input type='text' id='newTitle{$row['id']}' placeholder='Achievement Title' required>";
                    echo "<textarea id='newDescription{$row['id']}' placeholder='Achievement Description' required></textarea>";
                    echo "<button onclick='addAchievement({$row['id']})'>Save</button>";
                    echo "</div>";
                    echo "</section>";
                    ...
                    if ($gallery->num_rows > 0) {
                        while ($img = $gallery->fetch_assoc()) {
                            ...
                            echo "<p class='gallery-description'>{$img['description']}</p>";
                            echo "</div>";
                        }
                    } else {
                    ...
                    echo "</div>";
                    echo "<button class='add-gallery' data-athlete='{$row['id']}'>Add More Images</button>";
                    echo "<div class='new-gallery-form' id='newGalleryForm{$row['id']}' style='display:none;'>";
                    echo "<input type='file' id='newImage{$row['id']}' required>";
                    echo "<textarea id='newImageDescription{$row['id']}' placeholder='Image Description' required></textarea>";
                    echo "<button onclick='addGalleryImage({$row['id']})'>Save</button>";
                    echo "</div>";
                    echo "</section>";
            ...
            document.addEventListener("DOMContentLoaded", function () {
                document.querySelectorAll(".add-achievement").forEach(button => {
                    button.addEventListener("click", function () {
                        let athleteId = this.getAttribute("data-athlete");
                        document.getElementById("newAchievementForm" + athleteId).style.display = "block";
                    });
                });

                document.querySelectorAll(".add-gallery").forEach(button => {
                    button.addEventListener("click", function () {
                        let athleteId = this.getAttribute("data-athlete");
                        document.getElementById("newGalleryForm" + athleteId).style.display = "block";
                    });
                });
            });

            function addAchievement(athleteId) {
                let title = document.getElementById("newTitle" + athleteId).value;
                let description = document.getElementById("newDescription" + athleteId).value;
                ...
                if (title && description) {
                    ...
                    formData.append("action", "addAchievement");

                    fetch("handle_athletes.php", {
                        method: "POST",
                        body: formData
            ...
            function addGalleryImage(athleteId) {
                let imageInput = document.getElementById("newImage" + athleteId);
                let description = document.getElementById("newImageDescription" + athleteId).value;

                if (imageInput.files.length > 0 && description) {
                    let formData = new FormData();
                    formData.append("athlete_id", athleteId);
                    formData.append("image", imageInput.files[0]);
                    formData.append("description", description);
                    formData.append("action", "addGallery");
