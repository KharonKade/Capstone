    ...
    <table border="1">
        ...
        <tbody>
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                // Gallery (Display Filenames)
                echo "<td>";
                $gallery = $conn_content->query("SELECT image FROM athlete_gallery WHERE athlete_id='{$row['id']}'");
                while ($img = $gallery->fetch_assoc()) {
                    echo "<p>" . basename($img['image']) . "</p>"; // Display only filename
                }
                echo "</td>";

                // Edit Button
                echo "<td><button onclick=\"showEditModal('editAthleteForm{$row['id']}')\">Edit</button></td>";
                echo "</tr>";

                // Edit Form (Hidden Modal)
                echo "<div id='editAthleteForm{$row['id']}' class='modal' style='display:none;'>";
                echo "<div class='modal-content'>";
                echo "<span class='close' onclick=\"hideModal('editAthleteForm{$row['id']}')\">&times;</span>";
                echo "<h3>Edit Athlete</h3>";
                echo "<form method='post' action='handle_athletes.php' enctype='multipart/form-data'>";
                ...
                while ($img = $gallery->fetch_assoc()) {
                    echo "<p>" . basename($img['image']) . "</p>"; // Display only filename
                    echo "<input type='file' name='athlete_gallery[]'>"; // Option to upload new image
                }
                echo "</div>";

                echo "<button type='submit'>Update</button>";
                echo "<button type='button' onclick=\"hideModal('editAthleteForm{$row['id']}')\">Cancel</button>";
                echo "</form>";
                echo "</div>";
