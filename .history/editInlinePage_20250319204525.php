    ...
    <div class="admin-container">
        ...
        <main class="content">
                ...
                <table border="1">
                    ...
                    <tbody>
                        ...
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<h2>Edit Athlete</h2>";
                            echo "<form id='editAthleteForm{$row['id']}' method='post' action='handle_athletes.php' enctype='multipart/form-data'>";
                            ...
                            while ($ach = $achievements->fetch_assoc()) {
                                ...
                                echo "<button type='button' onclick=\"removeElement(this, 'achievement')\">Remove</button>";
                                echo "</div>";
                            }
                            echo "</div>";
                            ...
                            while ($img = $gallery->fetch_assoc()) {
                                ...
                                echo "<button type='button' onclick=\"removeElement(this, 'gallery-item')\">Remove</button>";
                                echo "</div>";
                            }
                            echo "</div>";
                            ...
                            echo "<button type='button' onclick=\"hideModal('editAthleteModal{$row['id']}')\">Cancel</button>";
                            echo "</form>";
                            echo "</div>";
                            echo "</div>";
            ...
            div.innerHTML = `
                ...
                <input type="text" name="achievements[]" placeholder="Achievement Title" required>
                <textarea name="achievements_descriptions[]" placeholder="Achievement Description" required></textarea>
                <button type="button" onclick="removeElement(this, 'achievement')">Remove</button>
            `;
            ...
            function addGalleryImage(containerId) {
                ...
                div.innerHTML = `
                    <input type="hidden" name="gallery_image_ids[]" value="new">
                    <input type="file" name="athlete_gallery[]" required>
                    <textarea name="gallery_descriptions[]" placeholder="Image Description" required></textarea>
                    <button type="button" onclick="removeElement(this, 'gallery-item')">Remove</button>
                `;
                container.appendChild(div);
            }

            function removeElement(button, className) {
                let parent = button.closest("." + className);
                if (parent) {
                    parent.remove();
                }
            }
