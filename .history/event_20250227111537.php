...
<header>
   ...
   document.addEventListener("DOMContentLoaded", function () {
    ...
    carousel.addEventListener("wheel", (e) => {
        ...
        let scrollAmount = e.deltaY * 1.5; // Adjust sensitivity for smooth feel
        scrollAmount = Math.sign(scrollAmount) * Math.max(5, Math.abs(scrollAmount)); // Ensure min movement

        // Animate scroll instead of setting it instantly
        const start = carousel.scrollLeft;
        const end = start + scrollAmount;
        const duration = 300; // Duration of animation (milliseconds)
        let startTime = null;

        function animateScroll(time) {
            if (!startTime) startTime = time;
            let progress = Math.min((time - startTime) / duration, 1);
            carousel.scrollLeft = start + (end - start) * easeOutQuad(progress);
