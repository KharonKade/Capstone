...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($edit_id) {
        ...
        $result = $conn->query("SELECT image FROM top_athletes WHERE id = '$edit_id'");
    ...
    if ($edit_id) {
        ...
        if ($conn->query($query) === TRUE) {
            // Update achievements
            $conn->query("DELETE FROM achievements WHERE athlete_id='$edit_id'");
            foreach ($achievements as $index => $achievement) {
                ...
                $conn->query("INSERT INTO achievements (athlete_id, title, description) VALUES ('$edit_id', '$achievement_title', '$achievement_desc')");
            }

            // Handle multiple gallery images without deleting existing ones
            if (!empty($_FILES["athlete_gallery"]["name"][0])) {
                foreach ($_FILES["athlete_gallery"]["tmp_name"] as $key => $tmp_name) {
                    if (!empty($_FILES["athlete_gallery"]["name"][$key]) && $_FILES["athlete_gallery"]["error"][$key] == 0) {
                        ...
                        $conn->query("INSERT INTO athlete_gallery (athlete_id, image, description) VALUES ('$edit_id', '$gallery_image_path', '$description')");
                    }
                }
            }
        ...
        if ($conn->query($query) === TRUE) {
            ...
            // Insert multiple gallery images
            if (!empty($_FILES["athlete_gallery"]["name"][0])) {
                foreach ($_FILES["athlete_gallery"]["tmp_name"] as $key => $tmp_name) {
                    if (!empty($_FILES["athlete_gallery"]["name"][$key]) && $_FILES["athlete_gallery"]["error"][$key] == 0) {
                        $gallery_image_path = "images/gallery/" . basename($_FILES["athlete_gallery"]["name"][$key]);
