...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["name"])) {
    ...
    if (isset($_POST["edit_id"])) {
        ...
        if (!$image) {
            $image = $existing_image; // Keep old image if no new one is uploaded
        }

        $query = "UPDATE top_athletes SET name=?, bio=?, description=?, wins=?, podium_finishes=?, years_active=?, specialty=?, image=? WHERE id=?";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("ssssssssi", $name, $bio, $description, $wins, $podium_finishes, $years_active, $specialty, $image, $id);
        $stmt->execute();
        ...
        if (!empty($_FILES["athlete_gallery"]["name"][0])) {
            ...
            foreach ($_FILES["athlete_gallery"]["tmp_name"] as $key => $tmp_name) {
                if (!empty($_FILES["athlete_gallery"]["name"][$key]) && $_FILES["athlete_gallery"]["error"][$key] == 0) {
                    ...
                    $desc = isset($_POST["gallery_descriptions"][$key]) ? $_POST["gallery_descriptions"][$key] : '';
                    $conn->query("INSERT INTO athlete_gallery (athlete_id, image, description) VALUES ('$id', '$gallery_image_path', '$desc')");
                }
            }
        }
    } else {
        // Inserting new athlete
        $query = "INSERT INTO top_athletes (name, bio, description, wins, podium_finishes, years_active, specialty, image) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($query);
        ...
        if (!empty($_FILES["athlete_gallery"]["name"][0])) {
            foreach ($_FILES["athlete_gallery"]["tmp_name"] as $key => $tmp_name) {
                if (!empty($_FILES["athlete_gallery"]["name"][$key]) && $_FILES["athlete_gallery"]["error"][$key] == 0) {
                    ...
                    move_uploaded_file($tmp_name, $gallery_image_path);
                    $desc = isset($_POST["gallery_descriptions"][$key]) ? $_POST["gallery_descriptions"][$key] : '';
                    $conn->query("INSERT INTO athlete_gallery (athlete_id, image, description) VALUES ('$athlete_id', '$gallery_image_path', '$desc')");
                }
            }
        }
    }
