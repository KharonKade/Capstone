...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST["edit_id"])) {
        ...
        if ($conn->query($query) === TRUE) {
            ...
            // Handle gallery images (only delete if new ones are uploaded)
            if (!empty($_FILES["athlete_gallery"]["name"][0])) {
                ...
                foreach ($_FILES["athlete_gallery"]["tmp_name"] as $key => $tmp_name) {
                    if ($_FILES["athlete_gallery"]["error"][$key] == 0) {
                        $image = "images/uploads/" . basename($_FILES["athlete_gallery"]["name"][$key]);
                        move_uploaded_file($tmp_name, $image);

                        $description = isset($_POST["gallery_descriptions"][$key]) ? $_POST["gallery_descriptions"][$key] : '';
                        $conn->query("INSERT INTO athlete_gallery (athlete_id, image, description) VALUES ('$id', '$image', '$description')");
                    }
                }
            }
            ...
            if (!empty($_FILES["athlete_gallery"]["name"][0])) {
                foreach ($_FILES["athlete_gallery"]["tmp_name"] as $key => $tmp_name) {
                    if ($_FILES["athlete_gallery"]["error"][$key] == 0) {
                        ...
                        $description = isset($_POST["gallery_descriptions"][$key]) ? $_POST["gallery_descriptions"][$key] : '';
                        $conn->query("INSERT INTO athlete_gallery (athlete_id, image, description) VALUES ('$athlete_id', '$image', '$description')");
                    }
                }
            }
