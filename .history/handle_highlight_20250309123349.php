...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["id"])) {
    ...
    // Update query without video
    $query = "UPDATE highlight_carousel SET title=?, description=? WHERE id=?";
    ...
    if (!empty($_FILES["video"]["name"])) {
        $video = "uploads/" . basename($_FILES["video"]["name"]);
        if (move_uploaded_file($_FILES["video"]["tmp_name"], $video)) {
            $query = "UPDATE highlight_carousel SET video=?, title=?, description=? WHERE id=?";
            $stmt = $conn->prepare($query);
